<?php
    ...
    if(!empty($_POST["send"])){
        ...
        if($_POST["delete_pass"] === $_POST["delete_key"]){
            ...
            // headerにtrue, 307を記載しpostで送られてきた値をリダイレクト先へ送信する
            header("Location: ./reply_edit.php", true, 307);        
        }
...
<body>
    ...
    <div class="border">
        <?php
            ...
            $delete_number = $_POST["delete_number"];
            $result = mysqli_query($link,"SELECT delete_key FROM replies WHERE id = $delete_number");
            while ($row = mysqli_fetch_assoc($result)){
                $delete_key = $row["delete_key"];
            }
        ?>
        <form action="./check_reply_edit_pass.php" method="post" enctype="multipart/form-data">
            <table>
                <tr>
                    <td>編集/削除キー:</td>
